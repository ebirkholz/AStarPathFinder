		...
		public void mousePressed(MouseEvent arg0) {
			...
			if(arg0.getButton()==1){
				if(mapa.mapa[my][mx]==0) {
					...
					//buscalargura.setaBusca(mapa, 0, 0,mx, my);
					//buscalargura.executaBusca();
...
// draw the current frame to an image buffer
{
	...
	if(buscalargura.osVisitados!=null) {
		synchronized (buscalargura.osVisitados) {
			for (Iterator iterator = buscalargura.osVisitados.iterator(); iterator.hasNext();) {
				Nodo oNodo = (Nodo) iterator.next();
				int nx = oNodo.x;
				int ny = oNodo.y;
	...
	if(buscalargura.osNodos!=null) {
		synchronized (buscalargura.osNodos) {
			for (Iterator iterator = buscalargura.osNodos.iterator(); iterator.hasNext();) {
				Nodo oNodo = (Nodo) iterator.next();
				int nx = oNodo.x;
				int ny = oNodo.y;
